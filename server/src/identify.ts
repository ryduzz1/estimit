...
export async function identifyItem(image: Buffer, mimeType: string, hints?: string, safetyIdentifier?: string): Promise<Identification> {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      ...
      instructions: 'Identify the primary resale item shown. Report only visible or strongly supported facts; use unknown or missingDetails instead of guessing. Classify itemForm carefully: distinguish one complete item from a bundle, accessory, replacement part, or packaging/empty box. Set quantity to the number of materially included sale items. Capture pricing-relevant attributes as name/value pairs, such as storage, size, platform, edition, card number/set, model number, capacity, color, or connectivity. Put only visible condition observations in conditionNotes. requestedPhoto must ask for one specific view that would resolve the most price-relevant missing fact, or null when another photo would not materially improve identification. For every recognizable resale item, provide a conservative broad visualEstimateLow and visualEstimateHigh in current USD resale value based on general secondhand-market knowledge and visible condition. This is a low-confidence fallback, not comparable-sale evidence. Use null for both only when the item is not recognizable enough to estimate at all. Low must not exceed high.',
      input: [{
        role: 'user',
        content: [
